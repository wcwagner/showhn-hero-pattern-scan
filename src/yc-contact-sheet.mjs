...
await fs.writeFile(`gallery/yc-${selection}-selection.json`,JSON.stringify({report:reportPath,selection,
  rule:selection==='audit'?'First 24 company-profile URLs ordered by SHA-256, independent of model/classifier output. Missing-data rows remain in sample.':`All ${selection} deterministic classifications; not visually verified labels.`,
